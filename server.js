...
var SampleApp = function() {
    ...
    self.removeFood = function(toRem) {
        while (toRem--) {
            food.pop();
        }
    }

    self.movePlayer = function (player) {
        var x =0,y =0;
    ...
    self.moveloop = function() {
        for (var i = 0; i < users.length; i++) {
            self.tickPlayer(users[i]);
        }
        for (i=0; i < massFood.length; i++) {
            if(massFood[i].speed > 0) self.moveMass(massFood[i]);
        }
    }
